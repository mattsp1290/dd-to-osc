...
def get_monitor_points(api_instance, query, scope, evaluation_function):
    end_time = int(time.time())  # int | End of the queried time period, seconds since the Unix epoch.
    start_time = end_time - time_modifier  # int | Start of the queried time period, seconds since the Unix epoch.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--monitor", type=int, default=None,
        help="The Datadog monitor to monitor")
    parser.add_argument("--scope", type=str, default=None,
        help="A scope for the monitor")
    parser.add_argument("--value", type=str, default=None,
        help="OSC channel to send values to")
    ...
    if not monitor:
        print("Must pass a monitor to monitor via --monitor or an environment variable named DATADOG_MONITOR")
        sys.exit(1)
    ...
    with ApiClient(configuration) as api_client:
        ...
        if comparitor != '>':
            ...
            sys.exit(1)
        threshold = monitor_response_dict.get('options').get('thresholds').get('critical')
        ...
        while True:
            evaluation = get_monitor_points(metrics_api_instance, query, scope, evaluation_function)
